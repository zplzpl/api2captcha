...
// Client allows programmatic access to the 2captcha api
type Client struct {
	key           string
	host          string
	client        *http.Client
...
// ClientOption is an option used to modify the 2captcha client
...
func WithTemplateName(value interface{}) OptionalValue {
	return func(m map[string]interface{}) {
		m["templateName"] = value
	}
}

func WithKey(value string) OptionalValue {
	return func(m map[string]interface{}) {
		if len(value) > 0 {
			m["clientKey"] = value
		}
...
func (c *Client) createTask(ctx context.Context, task interface{}) (taskId int64, err error) {
	...
	if response.ErrorId != 0 {
		err = fmt.Errorf("api2captcha_error,apikey: %s, code: %s,description: %s", c.key, response.ErrorCode, response.ErrorDescription)
		return
	}
...
func (c *Client) GetBalance(ctx context.Context) (balance float64, err error) {
	...
	if response.ErrorId != 0 {
		err = errors.New("api2captcha: " + response.ErrorDescription)
		return
	}
